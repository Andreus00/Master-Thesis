...
from scene_4ddress import IsotropicGaussianModel, GaussianModel, GaussianAvatar, GaussianAvatarCopy
...
def run_retarget(cfg, pipe):
    ...
    source_avatar = GaussianAvatarCopy(3, source_gaussians, source_smpl, source_smpl_pkl, source_scan_labels, RUN_NICP or LOAD_NICP)
    ...
    # source_avatar.lbs_gs(target_beta, target_theta)
    
    target_avatar = GaussianAvatarCopy(3, target_gaussians, target_smpl, target_smpl_pkl, target_scan_labels, RUN_NICP or LOAD_NICP)
    target_avatar.inverse_lbs()
    
    
    # 2. Call the transfer_clothes function to transfer the clothes from the source to the target
    
    # source_avatar.transfer_clothes_to(target_avatar, [3])
    
    # target_beta = torch.tensor(target_smpl_pkl["betas"], dtype=torch.float, device="cpu")
    # target_theta = torch.tensor(target_smpl_pkl["body_pose"], dtype=torch.float, device="cpu")
    # target_avatar.lbs_gs(target_beta, target_theta)
