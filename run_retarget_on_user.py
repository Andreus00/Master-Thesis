...
def run_NICP(cfg, pipe, src_clothes_label_ids, trg_clothes_label_ids):
    RUN_NICP = True
    LOAD_NICP = False
    LOAD_DATA_REG = False
    LOAD_DATA_REG_TARGET = False
    
    with open("smpl/smpl_vert_segmentation.json", "r") as f:
    ...
    if RUN_NICP:
        ...
        print(np.mean(target_smpl.vertices, axis=0))
        print(torch.mean(target_gaussians._xyz, dim=0))
        # exit()
        ...
        if not cfg.only_target_nicp:
            ...
            if not os.path.exists(src_nicp_path):
                os.makedirs(src_nicp_path)
            centers, total_size = register_shapes(source_trimesh, cfg.source_model, src_nicp_path, pipe_cfg, **model_pkg)
            source_trimesh.apply_scale(total_size)
            source_trimesh.apply_translation(centers)
            ...
            origin, xaxis = [0, 0, 0], [1, 0, 0]
            alpha = np.pi/2
            Rx = torch.asarray(trimesh.transformations.rotation_matrix(alpha, xaxis, origin)).float().cuda()
            
            # rotate gaussians by Rx
            xyz, rots = GaussianTransformUtils.rotate_by_matrix(source_gaussians._xyz, source_gaussians._rotation, Rx[:3, :3])
            source_gaussians._xyz = xyz
            source_gaussians._rotation = rots
        ...
        target_smpl.apply_transform(Rx.cpu())
    ...
    # Now use the smpl segmentation to keep the body parts that are visible
    smpl_body_parts_labels = ["leftLeg", "rightLeg", "leftUpLeg", "rightUpLeg"]
    
    for part in smpl_body_parts_labels:
        # get the indices of the vertices that belong to the body part
        part_idx = np.asarray(smpl_seg[part])
        
        # get the vertices of the body part
        part_vertices = torch.asarray(target_smpl.vertices[part_idx])
        colors = torch.ones((part_vertices.shape[0], 3), dtype=torch.float32) * 0.5
        normals = torch.ones((part_vertices.shape[0], 3), dtype=torch.float32)
        
        scales = torch.ones((part_vertices.shape[0], 1), dtype=torch.float32) * -7.0
        opacities = torch.ones((part_vertices.shape[0], 1), dtype=torch.float32)
        
        pcd = BasicPointCloud(points=part_vertices, colors=colors, normals=normals)
        
        gm = IsotropicGaussianModel(3)
        
        gm.create_from_pcd(pcd, 1, scales, opacities)
        
        target_gaussians._xyz = torch.cat([target_gaussians._xyz, gm._xyz], dim=0)
        target_gaussians._features_dc = torch.cat([target_gaussians._features_dc, gm._features_dc], dim=0)
        target_gaussians._features_rest = torch.cat([target_gaussians._features_rest, gm._features_rest], dim=0)
        target_gaussians._scaling = torch.cat([target_gaussians._scaling, gm._scaling], dim=0)
        target_gaussians._opacity = torch.cat([target_gaussians._opacity, gm._opacity], dim=0)
        target_gaussians._rotation = torch.cat([target_gaussians._rotation, gm._rotation], dim=0)
    ...
    for camera in [camera_front, camera_left, camera_right, camera_back]:
        ...
        if cfg.save_output:
            ...
            '''
            render_folder_name = "render" if RUN_NICP or LOAD_NICP else "render_on_smpl"
...
if __name__ == "__main__":
    ...
    for idx, target_model in enumerate(models):
        ...
        run_NICP(ep, pp.extract(args), args.src_clothes_label_ids, args.trg_clothes_label_ids)
